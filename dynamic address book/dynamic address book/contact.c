...
void init_address(con* pc)
{
	assert(pc);
	pc->count = 0;
	peoinfo*ptr= (peoinfo*)calloc(start,start* sizeof(peoinfo));
	if (ptr == NULL)
	{
		perror("init_address::calloc");
	}
	pc->data = ptr;
	pc->capacity= start;
}

void check_address(con* pc)
{
	assert(pc);
	//增加容量
	if (pc->count ==pc->capacity)
	{
		peoinfo* ptr = (peoinfo*)realloc(pc->data, (pc->capacity+ ING_COUNT) * sizeof(peoinfo));
		if (ptr == NULL)
		{
			perror("check_address::realloc");
			return;
		}
		pc->data = ptr;
		pc->capacity += ING_COUNT;
		printf("增容成功\n");
	}
	else
	{
...
{
	assert(pc);
	qsort(pc->data, pc->count, sizeof(peoinfo), name_sort);
}